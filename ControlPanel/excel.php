...
function listOfAluminis()
	...
	$myArr=array("Name","Year Of Passing","Currently Working In","Mobile No","Email Id");
	$excel->writeLine($myArr);
	
	$qry=mysqli_query($conn,"SELECT c.alumininame,c.aluminiyearofpassing, c.aluminicurrentlyworkingin, c.aluminimobileno, c.aluminiemailId
							  FROM tbl_alumini AS c") or die(mysql_error());
		if($qry!=false)
		{
			$i=1;
			while($res=mysqli_fetch_array($qry))
			{
				$myArr=array($res['alumininame'],$res['aluminiyearofpassing'],$res['aluminicurrentlyworkingin'],$res['aluminimobileno'],$res['aluminiemailId']);
		        $excel->writeLine($myArr);
				$i++;
			}
		}
